...
const keyLocation = './private.key.pem'
const certLocation = './domain.cert.pem'
...
try {
  FOLDERS = JSON.parse(fs.readFileSync('shared.json', 'utf8'))
...
stdin.addListener('data', (d) => { // input
  ...
  if (get[0] === 'get') {
    for (folder in FOLDERS) {
      q.info(folder +" "+ FOLDERS[folder])
    }
  }
  if (get[0] === 'sav') {
    var stream = fs.createWriteStream('shared.json')
